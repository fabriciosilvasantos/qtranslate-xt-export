...
class qTranslateXWidget extends WP_Widget {
	...
	function form( $instance ): void {
		...
		<p>
			...
			echo '</ul>';
			?>
			<small><?php printf( __( 'For example, format "%1$s" would do the same as the choice "%2$s".', 'qtranslate' ), esc_html( '%f<span>%n</span>' ), __( 'Text and Image', 'qtranslate' ) ); ?>
				&nbsp;
		...
		<p><label for="<?php echo $this->get_field_id( 'widget-css' ); ?>"><input type="checkbox"
			...
			<small><?php echo __( 'To reset to default, clear the text.', 'qtranslate' ) . ' ' . __( 'To disable this inline CSS, clear the check box.', 'qtranslate' ) . ' ' . sprintf( __( 'Other common CSS block for flag classes "%1$s" is loaded in the head of HTML and can be controlled with option "%2$s".', 'qtranslate' ), 'qtranxs_flag_xx', __( 'Head inline CSS', 'qtranslate' ) ); ?></small>
		</p>
		<?php
	}
}
