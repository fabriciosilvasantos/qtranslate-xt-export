...
function qtranxf_slugs_language_detect_redirect( $url_lang, $url_orig, $url_info ): string {
	...
	} elseif ( $q_config['hide_default_language'] && untrailingslashit( site_url() ) == untrailingslashit( $url_orig ) && untrailingslashit( $url_lang ) === untrailingslashit( wp_get_raw_referer() ) ) {
		return qtranxf_convertURL( $url_orig, $q_config['default_language'], false, true );
	}
