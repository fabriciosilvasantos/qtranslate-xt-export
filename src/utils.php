...
function qtranxf_parse_page_config( array $config, string $url_path, string $url_query ): array {
	...
	foreach ( $config as $pgkey => $pgcfg ) {
				...
				case 'forms':
					...
					foreach ( $cfg as $form_id => $pgcfg_form ) {
						...
						if ( is_string( $form_id ) ) {
							$id = $form_id;
						} elseif ( isset( $pgcfg_form['form']['id'] ) ) {
		...
		$page_configs[ $post_type_key ] = $page_config;
	}

	// Clean up empty configs
	foreach ( $page_configs as $post_type_key => $pg_cfg_item ) {
		if ( empty( $pg_cfg_item ) ) {
			unset( $page_configs[ $post_type_key ] );
			continue;
		}

		// Clean up 'fields' in forms
		if ( ! empty( $pg_cfg_item['forms'] ) && is_array( $pg_cfg_item['forms'] ) ) {
			foreach ( $pg_cfg_item['forms'] as $form_id => $form ) {
				if ( ! isset( $form['fields'] ) || ! is_array( $form['fields'] ) ) {
					continue;
				}
				foreach ( $form['fields'] as $k => $field ) {
					if ( qtranxf_set_field_jquery( $field ) ) {
						$pg_cfg_item['forms'][ $form_id ]['fields'][ $k ] = $field;
					}
				}
			}
		}

		// Remove any empty entries in the config item
		foreach ( $pg_cfg_item as $k => $cfg_val ) {
			if ( empty( $cfg_val ) ) {
				unset( $pg_cfg_item[ $k ] );
			}
		}
		...
		if ( empty( $pg_cfg_item ) ) {
			...
			$page_configs[ $post_type_key ] = $pg_cfg_item;
		}
	}
