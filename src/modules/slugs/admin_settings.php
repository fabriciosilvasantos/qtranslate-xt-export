...
function qtranxf_slugs_section_fn( string $section_id = '' ): void {
	switch ( $section_id ) {
		case 'post_types':
			echo '<p>' . sprintf(
				__( 'For example, the post_type <kbd>books</kbd> in Spanish would be displayed as <code>%1$s</code>. If you leave this blank will use the default option when you <a href="%2$s">registered</a> the post_type.', 'qtranslate' ),
		...
		case 'taxonomies':
			echo '<p>' . sprintf(
				__( 'For example, the taxonomy <kbd>category</kbd> in Spanish would be displayed as <code>%1$s</code>. If you leave this blank will use the default option when you <a href="%2$s">registered</a> the taxonomy (if you previously setup a base permastruct for <u>categories</u> or <u>tags</u> in <a href="%3$s">permalinks</a> page, these bases will be overwritten by the translated ones).', 'qtranslate' ),
				'https://example.org/es/categoria/taxonomy-name/',
...
function qtranxf_slugs_show_settings_page(): void {
	...
	<?php
	printf(
		__( 'If you activated previously the <a href="%1$s">pretty permalinks</a>, in this section you can translate the <abbr title="en inglés, Universal Resource Locator">URLs</abbr> <strong>bases</strong> for <a href="%2$s">public</a> post_types, categories, tags and taxonomies.', 'qtranslate' ),
		'options-permalink.php',
