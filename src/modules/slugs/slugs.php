...
class QTX_Module_Slugs {
	...
	function init() {
		...
		add_filter(
			...
			600,
			1
		); // TODO: if it is needed, this should be moved to main plugin...
	}
	 ...
	 */
	public function get_base_slug( ?string $name = null, ?string $lang = null ) {
		if ( ! $name || ! $lang ) {
			return false;
		}
	...
	public function query_vars( $query_vars ) {
		...
		if ( ! empty( $rewrite ) ) {
			...
			if ( isset( $wp->matched_rule ) && isset( $query ) && isset( $matches ) ) {
				...
				$matched_query = isset( $wp->matched_query ) && is_string( $wp->matched_query ) ? $wp->matched_query : '';
				parse_str( $matched_query, $query_vars_before_slugs_match );
				...
				$vars_to_purge      = array_filter(
					$vars_to_purge,
					function ( $var ) use ( $vars_slugs_to_keep ) {
				...
				$query_vars = array_filter(
					$query_vars,
					function ( $var ) use ( $vars_to_purge ) {
	...
	function filter_request( $query ) {
		...
		if ( empty( $query ) || isset( $query['error'] ) ) {
			...
			foreach ( $this->get_public_post_types() as $post_type ) {
				if ( array_key_exists( $post_type->name, $query ) && ! in_array(
					$post_type->name,
					array(
						'post',
						'page',
			...
			if ( isset( $query['post_type'] ) ) {
				if ( count( $query ) == 1 || ( count( $query ) == 2 && ! empty( $query['paged'] ) ) ) {
					...
					if ( $page instanceof WP_Post ) {
						$id          = $page->ID;
						/** @var array<int, WP_Post> $cache_array */
						$cache_array = array( $page );
			...
			foreach ( $this->get_public_taxonomies() as $item ) {
				if ( isset( $query[ $item->name ] ) ) {
					...
					$term      = ! empty( $term_slug ) ? $this->get_term_by( 'slug', $term_slug, $item->name ) : null;
					if ( $term instanceof WP_Term ) {
						/** @var array<int, WP_Term> $cache_array */
						$cache_array = array( $term );
	 ...
	 */
	public function get_extra_permastruct( ?string $permastruct = null, ?string $name = null ): string {
