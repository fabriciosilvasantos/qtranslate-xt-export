...
class QTX_Admin_Module_Manager {
	...
	public static function register_plugin_activated( string $activated_plugin ): void {
		...
		self::update_modules_state(
			function ( $check_plugin ) use ( $activated_plugin ) {
	...
	public static function register_plugin_deactivated( string $deactivated_plugin ): void {
		...
		self::update_modules_state(
			function ( $check_plugin ) use ( $deactivated_plugin ) {
		...
		foreach ( $modules as $module ) {
			...
			switch ( $options_modules[ $module->id ] ) {
				case QTX_MODULE_STATE_BLOCKED:
					...
					$msg                 = '<p>' . sprintf( __( 'The plugin "%1$s" is incompatible with the module "%2$s" of %3$s. Please disable it.', 'qtranslate' ), $plugin_name, $module->name, 'qTranslate&#8209;XT' ) . '</p>';
