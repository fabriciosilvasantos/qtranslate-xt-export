...
class QTX_Module_Acf_Admin {
	...
	public function admin_enqueue_scripts(): void {
		wp_enqueue_style(
			'qtranslate-acf',
			plugins_url( 'css/modules/acf.css', QTRANSLATE_FILE ),
			array( 'acf-input' ),
			QTX_VERSION
		);
		...
		wp_enqueue_script(
			'qtranslate-acf',
			plugins_url( 'dist/modules/acf.js', QTRANSLATE_FILE ),
			array(
				'acf-input',
				'jquery',
				'underscore',
	...
	public function filter_qtranslate_admin_config( array $config ): array {
		...
		$config['acf-post'] = array(
			...
			'forms' => array(
				...
				'wpbody-content' => array(
					'fields' => array(
						'acf-field-postbox' => array(
							'jquery' => '.acf-postbox .postbox-header h2',
							'encode' => 'display',
						),
						'acf-field-label'   => array(
		...
		$config['acf-field-group'] = array(
			...
			'forms'     => array(
				'post' => array(
					'fields' => array(
						'acf-field-group-object-label' => array(
		...
		$config['acf-forms'] = array(
			...
			'anchors' => array(
				'acf-form-data' => array( 'where' => 'after' ),
			),
			'forms'   => array(
				'wpbody-content' => array(
					'fields' => array(
						'acf-form-data-title'   => array(
		...
		$config['acf-taxonomy'] = array(
			...
			'anchors' => array(
				'acf-form-data' => array( 'where' => 'after' ),
				'edittag'       => array( 'where' => 'before' ), // To enforce the top LSB (usually wrap).
			),
			'forms'   => array(
				'wpbody-content' => array(
					'fields' => array(
						...
						'acf-field-input-description' => array(
	...
	public function update_settings(): void {
		...
		$post_acf = $_POST[ QTX_OPTIONS_MODULE_ACF ] ?? array();
		...
		array_walk( $options['standard_fields'], $set_bool, $post_acf['standard_fields'] ?? array() );
		array_walk( $options['group_sub_fields'], $set_bool, $post_acf['group_sub_fields'] ?? array() );
		array_walk( $options['qtranslate_fields'], $set_bool, $post_acf['qtranslate_fields'] ?? array() );
