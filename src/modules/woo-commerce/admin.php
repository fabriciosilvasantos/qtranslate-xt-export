...
function qtranxf_wc_add_filters_admin(): void {
	...
	qtranxf_add_filters( array( 'text' => $admin_hooks ) );
}
...
function qtranxf_wc_add_admin_page_config( array $page_configs ): array {
	...
	$fields['td-attribute_name']        = array(
		'jquery' => 'td.attribute_name',
		'encode' => 'display',
	);
	$fields['a-wc-order-item-name']     = array(
		'jquery' => 'a.wc-order-item-name',
		'encode' => 'display',
	);
	$fields['strong-attribute_name']    = array(
		'jquery' => 'strong.attribute_name',
		'encode' => 'display',
	);
	...
	$fields['display_meta']             = array(
		'jquery' => '.display_meta',
		'encode' => 'display',
	);
	$fields['select-option']            = array(
	...
	switch ( $pagenow ) {
		case 'admin.php':
			if ( isset( $_SERVER['QUERY_STRING'] ) && strpos( $_SERVER['QUERY_STRING'], 'page=wc-settings&tab=checkout' ) !== false ) {
				qtranxf_add_filters( array( 'text' => array( 'woocommerce_gateway_title' => 5 ) ) );
			}
			break;
		case 'edit.php':
			...
			if ( isset( $_SERVER['QUERY_STRING'] )
				...
				qtranxf_add_filters( array( 'text' => array( 'get_term' => 6 ) ) );
			}
			break;
	}
}
