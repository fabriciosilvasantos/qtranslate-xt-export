...
function qtranxf_wpseo_webpage_schema_meta( array $piece, $context ): array {
...
function qtranxf_wpseo_schema_organization( $data ): array {
	global $q_config;
...
function qtranxf_wpseo_schema_website( $data ): array {
	global $q_config;
...
function qtranxf_wpseo_json_ld_search_url( $search_url ): string {
	global $q_config;

	return qtranxf_convertURL( $search_url, $q_config['language'] );
}

function qtranxf_wpseo_schema_imageobject( $data ): array {
	global $q_config;
...
function qtranxf_wpseo_schema_webpage( $data ): array {
	global $q_config;
	...
	if ( isset( $data['potentialAction'][0]['target'][0] ) ) {
		$data['potentialAction'][0]['target'][0] = array( qtranxf_convertURL( $data['potentialAction'][0]['target'][0], $lang ) );
	}

	return $data;
}

function qtranxf_wpseo_schema_breadcrumb( $data ): array {
	global $q_config;
...
function qtranxf_wpseo_schema_person( $data ): array {
	global $q_config;
...
function qtranxf_wpseo_next_prev_filter( $link ): string {
	global $q_config;
...
function qtranxf_wpseo_add_filters_front(): void {
	...
	add_filter( 'wpseo_schema_webpage', 'qtranxf_wpseo_webpage_schema_meta', 10, 2 );
	...
	add_filter( 'wpseo_json_ld_search_url', 'qtranxf_wpseo_json_ld_search_url' );
