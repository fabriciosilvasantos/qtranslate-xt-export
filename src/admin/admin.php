	...
	if ( current_user_can( 'manage_options' ) ) {
		...
		if ( isset( $q_config['use_strftime'] ) ) {
			if ( $q_config['use_strftime'] != QTX_DATE_WP && ! class_exists( 'IntlDateFormatter' ) ) {
				...
				$warning .= sprintf(
					__( 'Class not found: <a href="%1$s">%2$s</a> likely due to missing PHP extension: <a href="%3$s">%4$s</a>.', 'qtranslate' ),
					'https://www.php.net/manual/en/class.intldateformatter.php',
					'`IntlDateFormatter`',
					'https://www.php.net/manual/en/intl.setup.php',
					'`Intl`'
				);
...
function qtranxf_get_admin_page_config_post_type( $post_type ) {
	...
	if ( ! empty( $page_config ) ) {
		...
		$page_config['js'][] = array(
			...
			'src'    => './dist/core.js',
			'deps'   => array( 'jquery', 'wp-deprecated', 'wp-hooks' ),
		);
		...
		foreach ( $page_config['js'] as $key => $js ) {
			...
			if ( $src[0] == '.' && ( $src[1] == '/' || $src[1] == DIRECTORY_SEPARATOR ) ) {
				...
				qtranxf_error_log( sprintf( __( 'Could not find script file "%1$s" for handle "%2$s".', 'qtranslate' ), $src, $js['handle'] ) );
			}
		}
	}
...
function qtranxf_admin_footer() {
	...
	if ( in_array( 'post', $config['page_config']['keys'] ?? array() ) ) {
...
function qtranxf_nav_menu_metabox( $object ) {
	...
	<div id="qtranxs-langsw" class="qtranxslangswdiv">
		...
		<p class="hide-if-js" id="qtranxs-langsw-help">
				...
				echo ' ';
				printf( __( 'The rendered menu items have CSS classes %1$s and %2$s ("%3$s" is a language code), which can be defined in theme style, if desired. The label of language menu can also be customized via field "%4$s" in the menu configuration.', 'qtranslate' ), '.qtranxs-lang-menu, .qtranxs-lang-menu-xx, .qtranxs-lang-menu-item', '.qtranxs-lang-menu-item-xx', 'xx', qtranxf_translate_wp( 'Navigation Label' ) );
				echo ' ';
				printf( __( 'The field "%1$s" of inserted menu item allows additional configuration described in our %2$sWiki%3$s.', 'qtranslate' ), qtranxf_translate_wp( 'URL' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Front-language-switching#language-switcher-menu" target="_blank">', '</a>' )
				?>
		</p>
	</span>
