...
function qtranxf_language_column( string $column ): string {
	...
	if ( $column == 'language' ) {
		...
		do_action_deprecated(
			'qtranslate_languageColumn',
			array(
				$available_languages,
				$missing_languages,
			),
			'3.10.0',
			'qtranslate_language_column'
		);
	}
...
function qtranxf_meta_box_LSB() {
	printf( __( 'This is a set of "%1$s" from %2$s. Click any blank space between the buttons and drag it to a place where you would need it the most. Click the handle at the top-right corner of this widget to hide this message.', 'qtranslate' ), __( 'Language Switching Buttons', 'qtranslate' ), '<a href="https://github.com/qTranslate/qtranslate-xt/" target="_blank">qTranslate&#8209;XT</a>' );
}
	...
	if ( current_user_can( 'manage_options' ) ) {
		...
		if ( $_REQUEST['filter_default'] ?? false ) {
			...
			foreach ( $default_opt_languages as $info_key ) {
				...
				$info_config[ $info_key ] = array_filter(
					$info_config[ $info_key ],
					function ( $value, $key ) use ( $default_values ) {
		...
		$info_config['admin_enabled_modules'] = array_keys(
			array_filter(
				$info_config['admin_enabled_modules'],
				function ( $value ) {
