			...
			foreach ( $main_config as $page_config ) {
				if ( isset( $page_config['js-conf'] ) ) {
					...
					if ( ! isset( $deprecated_js_configs[ $deprecated_key ] ) ) {
						$deprecated_js_configs[ $deprecated_key ] = array();
					}
...
function qtranxf_standardize_config_fields( array $fields ): array {
	foreach ( $fields as $key => $field ) {
		...
		} elseif ( qtranxf_set_field_jquery( $field ) ) {
			$fields[ $key ] = $field;
		}
	}
...
function qtranxf_normalize_config_files( array $file_paths ): array {
	...
	foreach ( $file_paths as $index => $path ) {
		...
		} elseif ( substr( $path, 0, $nc ) === WP_CONTENT_DIR ) {
...
function qtranxf_find_plugin_config_file( string $plugin ) {
	...
	if ( is_readable( $config_file ) ) {
		return qtranxf_normalize_config_files( array( $config_file ) )[0];
	}
...
function qtranxf_activation_hook(): void {
	...
	if ( version_compare( PHP_VERSION, '7.4' ) < 0 ) {
		...
		$msg = sprintf( __( 'Plugin %1$s requires PHP version %2$s at least. This server instance runs PHP version %3$s. A PHP version %4$s or higher is recommended. The plugin has not been activated.', 'qtranslate' ), qtranxf_get_plugin_link(), '7.4', PHP_VERSION, '8.4' );
