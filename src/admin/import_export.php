...
function qtranxf_migrate_import_mqtranslate(): void {
	...
	qtranxf_add_warning( sprintf( __( 'Option "%1$s" has also been turned on, as the most common case for importing configuration from %2$s. You may turn it off manually if your setup does not require it. Refer to %3$sFAQ%4$s for more information.', 'qtranslate' ), __( 'Compatibility Functions', 'qtranslate' ), $nm, '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/FAQ#compatibility-functions" target="_blank">', '</a>' ) );
}
...
function qtranxf_migrate_import( string $plugin_name, string $name_from ): void {
	...
	qtranxf_add_warning( sprintf( __( 'Applicable options and taxonomy names from plugin %1$s have been imported. Note that the multilingual content of posts, pages and other objects has not been altered during this operation. There is no additional operation needed to import content, since its format is compatible with %2$s.', 'qtranslate' ), $nm, 'qTranslate&#8209;XT' ) . ' ' . sprintf( __( 'It might be a good idea to review %1$smigration instructions%2$s, if you have not yet done so.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Migration-Guide" target="_blank">', '</a>' ) );
	qtranxf_add_warning( sprintf( __( '%1$sImportant%2$s: Before you start making edits to post and pages, please, make sure that both, your front site and admin back-end, work under this configuration. It may help to review "%3$s" and see if any of conflicting plugins mentioned there are used here. While the current content, coming from %4$s, is compatible with this plugin, the newly modified posts and pages will be saved with a new square-bracket-only encoding, which has a number of advantages comparing to former %5$s encoding. However, the new encoding is not straightforwardly compatible with %6$s and you will need an additional step available under "%7$s" option if you ever decide to go back to %8$s. Even with this additional conversion step, the 3rd-party plugins custom-stored data will not be auto-converted, but manual editing will still work. That is why it is advisable to create a test-copy of your site before making any further changes. In case you encounter a problem, please give us a chance to improve %9$s, send the login information to the test-copy of your site to %10$s along with a detailed step-by-step description of what is not working, and continue using your main site with %11$s meanwhile. It would also help, if you share a success story as well, either on %12$sthe forum%13$s, or via the same e-mail as mentioned above. Thank you very much for trying %14$s.', 'qtranslate' ), '<strong>', '</strong>', '<a href="https://github.com/qtranslate/qtranslate-xt/issues" target="_blank">' . 'Known Issues' . '</a>', $nm, 'qTranslate', $nm, '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Migration-Guide#convert-database" target="_blank"><strong>' . __( 'Convert Database', 'qtranslate' ) . '</strong></a>', $nm, 'qTranslate&#8209;XT', '[no mail support]', $nm, '<a href="https://github.com/qTranslate/qtranslate-xt/issues">', '</a>', 'qTranslate&#8209;XT' ) . '<br/>' . __( 'This is a one-time message, which you will not see again, unless the same import is repeated.', 'qtranslate' ) );
}

function qtranxf_migrate_export( string $plugin_name, string $name_to ): void {
	qtranxf_migrate_options_copy( $name_to, 'qtranslate' );
	$nm = '<strong>' . $plugin_name . '</strong>';
	qtranxf_add_message( sprintf( __( 'Applicable options have been exported to plugin %1$s. If you have done some post or page updates after migrating from %2$s, then "%3$s" operation is also required to convert the content to "dual language tag" style in order for plugin %4$s to function.', 'qtranslate' ), $nm, $nm, '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Migration-Guide/convert-database/" target="_blank"><strong>' . __( 'Convert Database', 'qtranslate' ) . '</strong></a>', $nm ) );
}
		...
		<tr id="qtranslate-convert-database">
			...
			<td>
				...
				<label for="qtranxs_convert_database_to_b_only"><input type="radio" name="convert_database"
					...
					<br/>
					<small><?php printf( __( 'The square bracket language tag %1$s only will be used as opposite to dual-tag (%2$s and %3$s) %4$s legacy database format. All string options and standard post and page fields will be uniformly encoded like %5$s.', 'qtranslate' ), '[:]', esc_html( '<!--:-->' ), '[:]', 'qTranslate', '"[:en]English[:de]Deutsch[:]"' ); ?></small>
				</label><br/><br/>
				...
				<label for="qtranxs_db_split"><input type="radio" name="convert_database" id="qtranxs_db_split"
					...
					&nbsp;
					<?php echo sprintf( __( 'Provide full file path to the input multilingual %1$s database file (the resulting %2$s files, named with language-based suffix, are saved in the same folder, where the input file is):', 'qtranslate' ), '.sql', '.sql' ); ?>
				</label><br/><input type="text" class="widefat" name="db_file" id="qtranxs_db_file"
									value="
									<?php
									if ( ! empty( $q_config['db_file'] ) ) {
				...
				<small>
				<?php
				echo sprintf( __( 'In order to remove one or more languages from entire database, you may dump the database into a %1$s file, then run this procedure and upload one of the new clean %2$s files back to the server. A separate clean database will also be saved fir each excluded language.', 'qtranslate' ), '.sql', '.sql' )// .sprintf(__('%sRead more%s.', 'qtranslate'), '&nbsp;<a href="">', '</a>')
				?>
					</small>
			</td>
		</tr>
