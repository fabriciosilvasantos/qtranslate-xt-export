...
class QTX_Admin_Settings {
	...
	public function display(): void {
		...
		<?php else : ?>
			...
			<p class="qtranxs_heading" style="font-size: small">
				<?php
				printf(
					__( 'For help on how to configure qTranslate correctly, take a look at the <a href="%1$s">qTranslate FAQ</a> and the <a href="%2$s">Support Forum</a>.', 'qtranslate' ),
		...
		<div class="form-wrap">
			<form action="<?php echo $form_action; ?>" id="qtranxs-edit-language" method="post"
				...
				<div class="form-field">
					...
					<p class="qtranxs-notes">
						<?php printf( __( 'Locale to be used in browser at front-end to set %1$s HTML attributes to specify alternative languages on a page. If left empty, then "%2$s" is used by default.', 'qtranslate' ), '"hreflang"', __( 'Language Code', 'qtranslate' ) ); ?>
						<br/>
					</p>
				</div>
				<div class="form-field">
					...
					<?php
					printf(
						__( 'Depending on your Date / Time Conversion Mode, you can either enter a <a href="%1$s">strftime</a> (use %2$s for day suffix) or <a href="%3$s">date</a> format. This field is optional. (Example: %4$s)', 'qtranslate' ),
						'https://www.php.net/manual/function.strftime.php',
						'%q',
				...
				<div class="form-field">
					...
					<?php
					printf(
						__( 'Depending on your Date / Time Conversion Mode, you can either enter a <a href="%1$s">strftime</a> or <a href="%2$s">date</a> format. This field is optional. (Example: %3$s)', 'qtranslate' ),
						'https://www.php.net/manual/function.strftime.php',
						'https://www.php.net/manual/function.date.php',
						'%I:%M %p'
					)
	...
	private function add_configuration_inspector(): void {
		...
		<p class="qtranxs_explanation">
			<?php
			printf( __( 'Review a combined JSON-encoded configuration as loaded from options %1$s and %2$s, as well as from the theme and other plugins via filters %3$s and %4$s.', 'qtranslate' ), '"' . __( 'Configuration Files', 'qtranslate' ) . '"', '"' . __( 'Custom Configuration', 'qtranslate' ) . '"', '"qtranslate_admin_config"', '"qtranslate_front_config"' );
			echo ' ';
			printf( __( 'Please, read %1$sIntegration Guide%2$s for more information.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Integration-Guide" target="_blank">', '</a>' );
			?>
			</p>
		<p class="qtranxs_explanation">
				...
				<td>
					<fieldset>
						...
						<label title="Pre-Path Mode">
							...
							/> 
							<?php
							echo sprintf( __( 'Use Pre-Path Mode (puts %s in front of URL)', 'qtranslate' ), '/' . ( $q_config['language'] ?? 'en' ) . '/' ) . '. ' . __( 'SEO friendly.', 'qtranslate' ) . ' [' . __( 'Default', 'qtranslate' ) . ']';
			...
			<tr>
				...
				<td>
					...
					<p class="qtranxs-notes"><?php _e( 'When checked, posts will be hidden if the content is not available for the selected language. If unchecked, a message will appear showing all the languages the content is available in.', 'qtranslate' ); ?>
						...
						<?php printf( __( 'This function will not work correctly if you installed %1$s on a blog with existing entries. In this case you will need to take a look at option "%2$s" under "%3$s" section.', 'qtranslate' ), 'qTranslate', __( 'Convert Database', 'qtranslate' ), __( 'Import', 'qtranslate' ) . '/' . __( 'Export', 'qtranslate' ) ); ?></p>
					<br/>
				...
				<td>
					<label for="filter_options_mode_all">
						<input type="radio" name="filter_options_mode"
								...
								?>
						/> <?php _e( 'Filter all WordPress options for translation at front-end. It may hurt performance of the site, but ensures that all options are translated.', 'qtranslate' ); ?> <?php _e( 'Starting from version 3.2.5, only options with multilingual content get filtered, which should help on performance issues.', 'qtranslate' ); ?>
					</label>
					<br/>
					<label for="filter_options_mode_list">
						<input type="radio" name="filter_options_mode"
								...
								?>
						/> <?php _e( 'Translate only options listed below (for experts only):', 'qtranslate' ); ?>
					</label>
					<br/>
					<input type="text" name="filter_options" id="qtranxs_filter_options"
							...
							style="width:100%">
					<p class="qtranxs-notes"><?php printf( __( 'By default, all options are filtered to be translated at front-end for the sake of simplicity of configuration. However, for a developed site, this may cause a considerable performance degradation. Normally, there are very few options, which actually need a translation. You may simply list them above to minimize the performance impact, while still getting translations needed. Options names must match the field "%1$s" of table "%2$s" of WordPress database. A minimum common set of option, normally needed a translation, is already entered in the list above as a default example. Option names in the list may contain wildcard with symbol "%3$s".', 'qtranslate' ), 'option_name', 'options', '%' ); ?></p>
				</td>
			</tr>
			...
			<tr id="option_lsb_style">
				...
				<td>
					<fieldset>
						...
						<br/><select name="lsb_style" id="lsb_style">
						<?php
	...
	private function add_integration_section( array $settings_modules ): void {
		...
		<table class="form-table qtranxs-form-table" id="qtranxs_integration_config">
			<tr>
				<td colspan="2"><p class="heading">
						<?php
						printf( __( 'If your theme or some plugins are not fully integrated with %1$s, suggest their authors to review the %2$sIntegration Guide%3$s. In many cases they would only need to create a simple text file in order to be fully compatible with %4$s. Alternatively, you may create such a file for them and for yourselves.', 'qtranslate' ), 'qTranslate&#8209;XT', '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Integration-Guide" target="_blank">', '</a>', 'qTranslate&#8209;XT' );
						echo ' ';
						printf( __( 'Read %1$sIntegration Guide%2$s for more information on how to customize the configuration of %3$s.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Integration-Guide" target="_blank">', '</a>', 'qTranslate&#8209;XT' );
						?>
					</p></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><label for="qtranxs_config_files"
							class="qtranxs_explanation"><?php printf( __( 'List of configuration files. Unless prefixed with "%1$s", paths are relative to %2$s variable: %3$s. Absolute paths are also acceptable.', 'qtranslate' ), './', 'WP_CONTENT_DIR', trailingslashit( WP_CONTENT_DIR ) ); ?></label>
					...
					printf( __( 'The list gets auto-updated on a 3rd-party integrated plugin activation/deactivation. You may also add your own custom files for your theme or plugins. File "%s" is the default configuration loaded from this plugin folder. It is not recommended to modify any configuration file from other authors, but you may alter any configuration item through your own custom file appended to the end of this list.', 'qtranslate' ), './i18n-config.json' );
						echo ' ';
						printf( __( 'Use "%1$s" to review the resulting combined configuration from all "%2$s" and this option.', 'qtranslate' ), '<a href="' . $this->options_uri . '&config_inspector=show' . '">' . __( 'Configuration Inspector', 'qtranslate' ) . '</a>', __( 'Custom Configuration', 'qtranslate' ) );
						echo ' ';
						printf( __( 'Please, read %1$sIntegration Guide%2$s for more information.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Integration-Guide" target="_blank">', '</a>' );
						echo ' ' . __( 'To reset to default, clear the text.', 'qtranslate' )
					?>
			...
			<tr>
				...
				<td><label for="qtranxs_json_custom_i18n_config"
							class="qtranxs_explanation <?php echo( $this->deprecated_class( 'custom_i18n_config' ) ); ?>">
							<?php
							_e( 'Deprecated.', 'qtranslate' );
							echo( '<br/>' );
							printf( __( 'Additional custom JSON-encoded configuration of %1$s for all admin pages. It is processed after all files from option "%2$s" are loaded, providing opportunity to add or to override configuration tokens as necessary.', 'qtranslate' ), 'qTranslate&#8209;XT', __( 'Configuration Files', 'qtranslate' ) );
							?>
						</label>
					<br/><textarea name="json_custom_i18n_config" id="qtranxs_json_custom_i18n_config"
									...
									if ( isset( $_POST['json_custom_i18n_config'] ) ) {
										...
										echo json_encode( $q_config['custom_i18n_config'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES );
									}
									?>
							</textarea>
					<p class="qtranxs-notes <?php echo( $this->deprecated_class( 'custom_i18n_config' ) ); ?>">
					<?php
					printf( __( 'It would make no difference, if the content of this field is stored in a file, which name is listed last in option "%s". Therefore, this field only provides flexibility for the sake of convenience.', 'qtranslate' ), __( 'Configuration Files', 'qtranslate' ) );
						...
						printf( __( 'Please, read %1$sIntegration Guide%2$s for more information.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Integration-Guide" target="_blank">', '</a>' );
						echo ' ';
						printf( __( 'Use "%1$s" to review the resulting combined configuration from all "%2$s" and this option.', 'qtranslate' ), '<a href="' . $this->options_uri . '&config_inspector=show">' . __( 'Configuration Inspector', 'qtranslate' ) . '</a>', __( 'Configuration Files', 'qtranslate' ) );
					?>
						</p>
				</td>
			</tr>
			<tr>
				...
				<td><p class="qtranxs_explanation">
						<?php printf( __( 'Enter "%1$s" or "%2$s" attribute of text fields from your theme, which you wish to translate. This applies to post, page and media editors (%3$s). To lookup "%4$s" or "%5$s", right-click on the field in the post or the page editor and choose "%6$s". Look for an attribute of the field named "%7$s" or "%8$s". Enter it below, as many as you need, space- or comma-separated. After saving configuration, these fields will start responding to the language switching buttons, and you can enter different text for each language. The input fields of type %9$s will be parsed using %10$s syntax, while single line text fields will use %11$s syntax. For more information, read %12$sFAQ%13$s.', 'qtranslate' ), 'id', 'class', '/wp-admin/post*', 'id', 'class', _x( 'Inspect Element', 'browser option', 'qtranslate' ), 'id', 'class', '\'textarea\'', esc_html( '<!--:-->' ), '[:]', '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/FAQ#custom-fields">', '</a>' ); ?></p>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><label for="qtranxs_text_field_filters" class="qtranxs_explanation">
						<input type="text" name="text_field_filters" id="qtranxs_text_field_filters"
								...
								style="width:100%"></label>
					<p class="qtranxs-notes"><?php printf( __( 'Names of filters (which are enabled on theme or other plugins via %1$s function) to add translation to. For more information, read %2$sFAQ%3$s.', 'qtranslate' ), 'apply_filters()', '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/FAQ#custom-fields">', '</a>' ); ?></p>
				</td>
			</tr>
			<tr>
		...
		<?php
		$this->close_section( 'integration' );
	}
			...
			<div id="col-right">
				<div class="col-wrap">
					...
					<?php
						...
						echo ' ';
						printf( __( 'You may %1$s or %2$s a language, or %3$s manually added language, or %4$s previous modifications of a pre-defined language.', 'qtranslate' ), '"' . __( 'Enable', 'qtranslate' ) . '"', '"' . __( 'Disable', 'qtranslate' ) . '"', '"' . __( 'Delete', 'qtranslate' ) . '"', '"' . __( 'Reset', 'qtranslate' ) . '"' );
						echo ' ';
