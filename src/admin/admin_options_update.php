...
function qtranxf_edit_config(): void {
	...
	if ( isset( $_POST['original_lang'] ) ) {
		...
		if ( empty( $errors ) ) {
			...
			$messages[] = sprintf( __( 'The new language properties have been saved. If you think these properties should be the preset default, please %1$ssend email%2$s to the development team.', 'qtranslate' ), '<a href="mailto:' . $u . '"><strong>', '</strong></a>' );
		}
...
function qtranxf_update_setting( $var, int $type = QTX_STRING, $def = null ): bool {
	...
	switch ( $type ) {
		...
		case QTX_STRING:
			...
			} elseif ( $type == QTX_LANGUAGE && ! qtranxf_isEnabled( $val ) ) {
				return false;
			}
