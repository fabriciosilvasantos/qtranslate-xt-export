...
function qtranxf_admin_notice_config_files_changed(): void {
	...
	printf( __( 'Option "%1$s" for plugin %2$s has been auto-adjusted after recent changes in the site configuration. It might be a good idea to %3$sreview the changes%4$s in the list of configuration files.', 'qtranslate' ), '<a href="' . $url . '">' . __( 'Configuration Files', 'qtranslate' ) . '</a>', qtranxf_get_plugin_link(), '<a href="' . $url . '">', '</a>' );
	echo '<br/></p><p>';
...
function qtranxf_admin_notices_block_editor(): void {
	...
	<div class="notice notice-warning qtranxs-notice-ajax is-dismissible" id="qtranxs-gutenberg-support">
		...
		<?php
		if ( ! qtranxf_is_classic_editor_supported() ) :
			$link_classic = admin_url( 'plugin-install.php?tab=plugin-information&plugin=classic-editor' );
			$link_plugins = admin_url( 'plugins.php' );
			?>
			<p><?php printf( __( 'It is recommended to install the <a href="%1$s">%2$s</a> in your <a href="%3$s">plugins</a>.', 'qtranslate' ), $link_classic, 'Classic Editor', $link_plugins ); ?></p>
		<?php endif; ?>
		<p>
...
function qtranxf_admin_notices_slugs_migrate(): void {
	...
	echo '<p>' . sprintf( __( '%1$s : found slugs meta that can be migrated. Go to the <a href="%2$s">import settings</a> to migrate.', 'qtranslate' ), qtranxf_get_plugin_link(), $options_link ) . '</p>';
	echo '<p>' . $msg . '</p>';
...
function qtranxf_admin_notice_deactivate_plugin( $name, $plugin ): void {
	...
	$s   = '</p><p>' . sprintf( __( 'It might be a good idea to review %1$smigration instructions%2$s, if you have not yet done so.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Migration-Guide/" target="_blank">', '</a>' ) . '</p><p><a class="button" href="">';
	$msg = sprintf( __( 'Activation of plugin %1$s deactivated plugin %2$s since they cannot run simultaneously.', 'qtranslate' ), $qtxlink, $link ) . ' ';
	if ( $imported ) {
		$msg .= sprintf( __( 'The compatible settings from %1$s have been imported to %2$s. Further tuning, import, export and reset of options can be done at Settings/Languages configuration page, once %3$s is running.%4$sContinue%5$s', 'qtranslate' ), $name, $qtxnm, $qtxnm, $s, '</a>' );
	} else {
		$msg .= sprintf( __( 'You may import/export compatible settings from %1$s to %2$s on Settings/Languages configuration page, once %3$s is running.%4$sContinue%5$s', 'qtranslate' ), $name, $qtxnm, $qtxnm, $s, '</a>' );
	}
	wp_die( '<p>' . $msg . '</p>' );
}
...
function qtranxf_admin_notice_plugin_conflict( $title, $plugin ): void {
	...
	printf( __( '%1$sError:%2$s plugin %3$s cannot run concurrently with plugin %4$s. You may import and export compatible settings between %5$s and %6$s on Settings/<a href="%7$s">Languages</a> configuration page. Then you have to deactivate one of the plugins to continue.', 'qtranslate' ), '<strong>', '</strong>', $me, $link, 'qTranslate&#8209;XT', $title, admin_url( 'options-general.php?page=qtranslate-xt' ), 'qtranslate' );
	echo ' ';
	printf( __( 'It might be a good idea to review %1$smigration instructions%2$s, if you have not yet done so.', 'qtranslate' ), '<a href="https://github.com/qtranslate/qtranslate-xt/wiki/Migration-Guide/" target="_blank">', '</a>' );
...
function qtranx_admin_deprecated_settings(): array {
	...
	return array(
		...
		'use_strftime_date_override'     => array(
		...
		'use_strftime_strftime_override' => array(
			'check'   => $q_config['use_strftime'] == QTX_STRFTIME_OVERRIDE,
			'name'    => __( 'Date / Time Conversion', 'qtranslate' ),
			'section' => 'advanced',
			'hint'    => '<a href="https://github.com/qtranslate/qtranslate-xt/issues/1234">github #1234</a>',
		),
		'custom_fields_format'           => array(
		...
		'custom_field_classes_format'    => array(
...
function qtranxf_admin_notices_deprecated_settings(): void {
	...
	foreach ( qtranx_admin_deprecated_settings() as $setting_key => $setting_data ) {
		...
		if ( $setting_data['check'] ) {
			...
			$message .= sprintf(
				__( 'The value set for option "%1$s" is deprecated, it will not be supported in the future. Go to "%2$s" settings to change it.', 'qtranslate' ),
				'<strong>' . $setting_data['name'] . '</strong>',
				sprintf( '<a href="%s">%s</a>', $section_url, $section_name )
			);
