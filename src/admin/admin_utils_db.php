...
function qtranxf_split_database_file( string $ifp, array $languages_to_keep ): string {
	...
	return sprintf( __( 'The database file provided has been split as requested. Number of multilingual strings found is %1$s. The result files are:%2$s', 'qtranslate' ), $cnt, $fns );
}
...
function qtranxf_db_clean_terms(): string {
	...
	foreach ( $result as $row ) {
		...
		if ( ! ( $term instanceof WP_Term ) ) {
			...
			$messages[] = sprintf( __( 'Term "%1$s" (id=%2$d) cannot be loaded and is left untouched. Error message on load was:%3$s', 'qtranslate' ), $nm, $id, '<br/>' . PHP_EOL . '"' . implode( '"<br/>"' . PHP_EOL, $errs ) . '"' ) . '<br/>';
			continue;
		} else {
		...
		} elseif ( ! empty( $term_name_cur[ $nm ] ) && is_array( $term_name_cur[ $nm ] ) ) {
			$ts = $term_name_cur[ $nm ];
		} else {
			continue;
		}
		...
		$msgs[] = sprintf(
			__( 'Term "%1$s" (id=%2$d) has been modified from:%3$sto:%4$s', 'qtranslate' ),
			esc_html( $nm ),
			$id,
